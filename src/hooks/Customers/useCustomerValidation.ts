...
export const useCustomerValidation = () => {
  ...
  const [customerValidationErrors, setCustomerValidationErrors] = useState<any>(
    {}
  );
  ...
  const customerValidations = (values: Customer) => {
    ...
    validationErrors.FirstName = validationsTypes.requiredField(
      values.FirstName,
      "FirstName"
    );
    validationErrors.City = validationsTypes.requiredField(values.City, "City");
    validationErrors.State = validationsTypes.requiredField(
      values.State,
      "State"
    );
    validationErrors.ZipCode = validationsTypes.nullOrLengthEqualTo(
      values.ZipCode,
      "ZipCode",
      5
    );
